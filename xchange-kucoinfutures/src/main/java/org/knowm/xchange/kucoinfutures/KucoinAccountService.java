...
public class KucoinAccountService extends KucoinAccountServiceRaw implements AccountService {
  ...
  public AccountInfo getAccountInfo() throws IOException {
    AccountBalancesResponse account = getKucoinAccount();
    return new AccountInfo(
      Wallet.Builder.from(Arrays.asList(KucoinAdapters.adaptBalance(account))).build()
    );
  }
    ...
    if (withdrawals) {
      result.addAll(
          getWithdrawalsList(currency, null, startAt, endAt, null, null).getItems().stream()
              .map(org.knowm.xchange.kucoinfutures.KucoinAdapters::adaptFundingRecord)
