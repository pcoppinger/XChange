...
import org.knowm.xchange.kucoinfutures.service.QueryOrderParamClientOid;
...
public class KucoinTradeServiceRaw extends KucoinBaseService {
  ...
  public OrderResponse getKucoinOrder(OrderQueryParams params) throws IOException {
    checkAuthenticated();
    if (params instanceof QueryOrderParamClientOid) {
      return classifyingExceptions(
              () -> orderApi.getOrderByClientOid(apiKey, digest, nonceFactory, passphrase, params.getOrderId()));
    } else {
      return classifyingExceptions(
              () -> orderApi.getOrder(apiKey, digest, nonceFactory, passphrase, params.getOrderId()));
    }
  }
